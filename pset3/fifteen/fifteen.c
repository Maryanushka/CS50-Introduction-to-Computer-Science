...
void init(void)
{
    //set board values based on user-defined "cell"
    for (int row = 0; row < d; row++)
    {
        for (int col = 0; col < d; col++)
        {
            board[row][col] = (d * d - d * row) - col - 1;
        }
    }

    //if the board sides are even, swap 1 & 2
    if (d % 2 == 0)
    {
        board[d - 1][d - 2] = 2;
        board[d - 1][d - 3] = 1;
...
void draw(void)
{
    //loop over the board values to print the board; replace 0 with "_"
    for (int row = 0; row < d; row++)
    {
        for (int col = 0; col < d; col++)
        {
